...
object Maybe {
  ...
  implicit val maybeApplicative: Applicative[Maybe] = new Applicative[Maybe] {
    ...
    override def fmap[A, B](f: A => B)(fa: Maybe[A]): Maybe[B] = maybeFunctor.fmap(f)(fa)
  }
  ...
  implicit val maybeMonad: Monad[Maybe] = new Monad[Maybe] {
    ...
    override def join[A, B](mma: Maybe[Maybe[A]]): Maybe[A] = bind(mma)(identity)
    override def ap[A, B](ff: Maybe[A => B])(fa: Maybe[A]): Maybe[B] = maybeApplicative.ap(ff)(fa)
    override def pure[A](a: A): Maybe[A] = maybeApply.pure(a)
    override def fmap[A, B](f: A => B)(fa: Maybe[A]): Maybe[B] = maybeFunctor.fmap(f)(fa)
  }
  ...
  implicit val maybeTraversable: Traversable[Maybe] = new Traversable[Maybe] {
    ...
    override def traverse[A, B, F[_]](k: A => F[B])(ta: Maybe[A])(implicit A: Applicative[F]): F[Maybe[B]] =
      sequenceA[B, F](fmap(k)(ta))

    override def sequenceA[A, F[_]](tfa: Maybe[F[A]])(implicit A: Applicative[F]): F[Maybe[A]] =
      traverse[F[A], A, F](identity)(tfa)

    override def foldr[A, B](f: (A, B) => B)(z: B)(fa: Maybe[A]): B = maybeFoldable.foldr(f)(z)(fa)
    override def fmap[A, B](f: A => B)(fa: Maybe[A]): Maybe[B] = maybeFunctor.fmap(f)(fa)
  }
}
