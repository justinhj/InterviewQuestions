...
object IdExamples {
  ...
  implicit val idTraversable: Traversable[Id] = new Traversable[Id] {
    override def traverse[A, B, F[_]](k: A => F[B])(ta: Id[A])(
        implicit A: Applicative[F]): F[Id[B]] =
      k(ta)
    override def sequenceA[A, F[_]](tfa: Id[F[A]])(
        implicit A: Applicative[F]): F[Id[A]] =
      traverse[F[A], A, F](identity)(tfa)

    override def foldr[A, B](f: (A, B) => B)(z: B)(fa: Id[A]): B =
      idFoldable.foldr(f)(z)(fa)
    override def fmap[A, B](f: A => B)(fa: Id[A]): Id[B] = idFunctor.fmap(f)(fa)
  }
